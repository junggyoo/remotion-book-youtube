 ...
 */
export const GENRE_STRATEGY_AFFINITY: Record<GenreKey, Partial<Record<HookStrategy, number>>> = {
...
export const HOOK_STRATEGIES: Record<HookStrategy, HookStrategyDef> = {
  pain: {
    ...
    score(fp) {
      ...
      const s = (fp.structure === 'narrative' || fp.structure === 'argument' ? 0.8 : 0.3) * W.structure
      const t = makeToneChecker(['urgent', 'intense', 'provocative'])(fp) * W.tone
      const c = (fp.contentMode === 'actionable' || fp.contentMode === 'mixed' ? 0.7 : 0.3) * W.contentMode
      const n = (fp.narrativeArcType === 'warning' ? 0.9 : fp.narrativeArcType === 'transformation' ? 0.5 : 0.2) * W.narrativeArc
      return g + s + t + c + n
    },
  },
  ...
  contrarian: {
    ...
    score(fp) {
      ...
      const s = (fp.structure === 'argument' ? 0.9 : fp.structure === 'collection' ? 0.6 : 0.3) * W.structure
      const t = makeToneChecker(['provocative', 'intense', 'reflective'])(fp) * W.tone
      const c = (fp.contentMode === 'conceptual' ? 0.8 : fp.contentMode === 'mixed' ? 0.5 : 0.3) * W.contentMode
      const n = (fp.narrativeArcType === 'discovery' ? 0.8 : fp.narrativeArcType === 'warning' ? 0.6 : 0.2) * W.narrativeArc
      return g + s + t + c + n
    },
  },
  ...
  transformation: {
    ...
    score(fp) {
      ...
      const s = (fp.structure === 'framework' ? 0.7 : fp.structure === 'narrative' ? 0.8 : 0.3) * W.structure
      const t = makeToneChecker(['uplifting', 'hopeful', 'disciplined'])(fp) * W.tone
      const c = (fp.contentMode === 'actionable' ? 0.8 : fp.contentMode === 'narrative' ? 0.6 : 0.3) * W.contentMode
      const n = (fp.narrativeArcType === 'transformation' ? 0.95 : fp.narrativeArcType === 'instruction' ? 0.4 : 0.2) * W.narrativeArc
      return g + s + t + c + n
    },
  },
  ...
  identity: {
    ...
    score(fp) {
      ...
      const s = (fp.structure === 'narrative' ? 0.7 : fp.structure === 'argument' ? 0.5 : 0.3) * W.structure
      const t = makeToneChecker(['reflective', 'calm', 'hopeful'])(fp) * W.tone
      const c = (fp.contentMode === 'conceptual' ? 0.7 : fp.contentMode === 'narrative' ? 0.6 : 0.3) * W.contentMode
      const n = (fp.narrativeArcType === 'discovery' ? 0.7 : fp.narrativeArcType === 'transformation' ? 0.5 : 0.2) * W.narrativeArc
      return g + s + t + c + n
    },
  },
  ...
  question: {
    ...
    score(fp) {
      ...
      const s = (fp.structure === 'argument' ? 0.8 : fp.structure === 'collection' ? 0.6 : 0.4) * W.structure
      const t = makeToneChecker(['reflective', 'provocative', 'calm'])(fp) * W.tone
      const c = (fp.contentMode === 'conceptual' ? 0.9 : fp.contentMode === 'mixed' ? 0.5 : 0.3) * W.contentMode
      const n = (fp.narrativeArcType === 'discovery' ? 0.9 : fp.narrativeArcType === 'warning' ? 0.4 : 0.2) * W.narrativeArc
      return g + s + t + c + n
    },
  },
  ...
  system: {
    ...
    score(fp) {
      ...
      const s = (fp.structure === 'framework' && fp.coreFramework ? 0.95 : fp.structure === 'framework' ? 0.7 : 0.3) * W.structure
      const t = makeToneChecker(['disciplined', 'intense', 'urgent'])(fp) * W.tone
      const c = (fp.contentMode === 'actionable' ? 0.85 : fp.contentMode === 'mixed' ? 0.5 : 0.2) * W.contentMode
      const n = (fp.narrativeArcType === 'instruction' ? 0.8 : fp.narrativeArcType === 'transformation' ? 0.6 : 0.2) * W.narrativeArc
      return g + s + t + c + n
    },
  },
  ...
  urgency: {
    ...
    score(fp) {
      ...
      const s = (fp.structure === 'argument' ? 0.7 : fp.structure === 'narrative' ? 0.5 : 0.3) * W.structure
      const t = makeToneChecker(['urgent', 'intense', 'provocative'])(fp) * W.tone
      const c = (fp.contentMode === 'actionable' ? 0.7 : fp.contentMode === 'mixed' ? 0.6 : 0.3) * W.contentMode
      const n = (fp.narrativeArcType === 'warning' ? 0.95 : fp.narrativeArcType === 'transformation' ? 0.4 : 0.2) * W.narrativeArc
      return g + s + t + c + n
    },
  },
}
 ...
 */
export function selectHookStrategy(fingerprint: BookFingerprint): HookSelection {
  ...
  for (const s of scores) {
    if (s.strategy !== primary.strategy && s.score >= primary.score * SECONDARY_THRESHOLD) {
...
function createHookBlueprint(
  ...
  const elements: VCLElement[] = [
    { id: 'hook-headline', type: 'headline', props: { text: fingerprint.entryAngle } },
    { id: 'hook-body', type: 'body-text', props: { text: strategyDef.pattern } },
...
function createIntroBlueprint(
  ...
  const elements: VCLElement[] = [
    ...
    { id: 'intro-image', type: 'image', props: { src: '', role: 'book-cover' } },
  ]
...
export function composeOpening(
  ...
  const packageDurationSec = Math.max(totalRange[0], Math.min(totalRange[1], rawDurationSec))
